...
def read_dataset():
    ...
    label_list = []
    i=0
    my_list = os.listdir(r'static\\train_image\\')
    for pa in my_list:

        print(pa,"==================",i)
        for root, dirs, files in os.walk(r'static\\train_image\\' + pa):

         for f in files:
            file_path = os.path.join(r'static\\train_image\\'+pa, f)
            # print(file_path)
...
if not os.path.exists("cyber_bul.h5"):

    model.fit_generator(train_generator, steps_per_epoch=batch_size, epochs=epochs)
    model.save("cyber_bul.h5")  # train for randomly selected one
else:
    model = load_model("cyber_bul.h5")  # load weights
